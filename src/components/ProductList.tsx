...
import { ProductCard, ProductCard3, ProductCardType2 } from "./ProductCard";
...
export const ProductsGrid = ({
  ...
  // Autoplay sequence for grid videos
  useEffect(() => {
    if (!products.length) return;
    ...
    const timer = setInterval(() => {
      // Skip items with no video
      let nextIndex = (currentIndex + 1) % products.length;
      let attempts = 0;

      while (!products[nextIndex].video && attempts < products.length) {
  ...
  return (
    <div className="relative">
      <div className="bg-white py-3 md:py-6 mx-auto px-2 ">
        ...
        <div className=" flex  items-center pl-2 mt-1 py-2 justify overflow-x-auto gap-2 snap-x snap-mandatory scroll-smooth scrollbar-hide -mb-2">
          {[
            {
              title: "Crazy Deal",
              key: "crazyDeal",
...
export const ProductList3 = ({
  ...
  const { ref: sectionRef, inView } = useInView({
    threshold: 0.3, // section visible threshold
  });
  ...
  useEffect(() => {
    ...
    if (activeCard) {
      activeCard.scrollIntoView({
        behavior: "smooth",
        block: "nearest", // <-- prevents vertical scroll adjustment
        inline: "center",
      });
    }
  }, [activeIndex]);
